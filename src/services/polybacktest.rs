            ...
            let markets = response
                ...
                .ok_or_else(|| {
                    AppError::InvalidMarket(
                        "PolyBackTest did not return a markets array".to_owned(),
                    )
                })?;
    ...
    async fn fetch_snapshots_in_window(
        ...
        loop {
            ...
            let page = response
                ...
                .ok_or_else(|| {
                    AppError::InvalidMarket(
                        "PolyBackTest did not return a snapshots array".to_owned(),
                    )
                })?;
...
impl PolyBacktestMarket {
    fn from_value(value: &Value) -> Result<Option<Self>> {
        ...
        let market_id = extract_string(value, &["market_id", "id"]).ok_or_else(|| {
            AppError::InvalidMarket("PolyBackTest market_id is missing".to_owned())
        })?;
        let start_time =
            extract_datetime(value, &["start_time", "startTime"]).ok_or_else(|| {
                AppError::InvalidMarket("PolyBackTest start_time is missing".to_owned())
            })?;
        let end_time = extract_datetime(value, &["end_time", "endTime"]).ok_or_else(|| {
            AppError::InvalidMarket("PolyBackTest end_time is missing".to_owned())
        })?;
...
fn resolve_polybacktest_api_key(config: &PolyBacktestConfig) -> Result<String> {
    ...
    prompt_line(
        "PolyBackTest API key (you can paste it here): ",
        &config.api_key_env,
    )
}
