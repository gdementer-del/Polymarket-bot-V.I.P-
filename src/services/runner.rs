...
use self::dashboard::PaperRunDashboardWriter;
use self::price_monitor::run_price_monitor;
...
use super::chainlink::ChainlinkOracleTriggerEvent;
...
async fn run_loop(
    ...
    let mut paper_dashboard =
        (mode == BotMode::Paper && control.status_dashboard).then(PaperRunDashboardWriter::spawn);
    ...
    let mut processed_cycle_count = 0_usize;
    loop {
        ...
        if mode == BotMode::Paper {
            ...
            processed_cycle_count = processed_cycle_count.saturating_add(1);
            cycle_entry.processed_cycle_count = processed_cycle_count;
            ...
            if let Some(dashboard) = paper_dashboard.as_mut() {
                dashboard.observe_cycle(
                    &cycle_entry,
                    run_started_at,
                    max_runtime,
                    drain_started_at.is_some(),
                );
            }
    ...
    if config.run.polymarket_stream.enabled {
        data_client.stop_market_stream();
    }
...
fn show_paper_run_summary(
    ...
    let processed_cycles = cycles.last().map_or(cycles.len(), |cycle| {
        cycle.processed_cycle_count.max(cycles.len())
    });
    ...
    println!(
        "Paper run cycles: processed_cycles={} sampled_cycles={} signals={} exec_cycles={} near_miss_cycles={} near_miss_events={} risk_blocked={} last_cycle={}",
        processed_cycles,
        cycles.len(),
        cycles_with_signals,
        cycles_with_exec,
...
fn log_paper_cycle_summary(entry: &PaperCycleEntry) {
    debug!(
        processed_cycles = entry.processed_cycle_count,
...
fn log_auth_check(report: &AuthCheckReport) {
    info!(
        ...
        api_key_present = !report.api_key.is_empty(),
