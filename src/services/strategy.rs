...
impl BundleArbitrageStrategy {
    ...
    fn build_bonereaper_state_v2_variant_opportunity(
        ...
        .then(|| codex_scalp_probe_v1_raw_light_profile_for_config(context.target, &self.config));
        ...
        if kind == OpportunityKind::CodexScalpProbeV1
            && !codex_scalp_probe_v1_fill_ratio_allows(
                primary_fill.total_cost,
                max_notional,
                &self.config,
            )
        {
    ...
    fn evaluate_codex_scalp_probe_v1_near_miss(
        ...
        if codex_scalp_probe_v1_raw_light_mode(&self.config)
            && !context.target_price_source.is_oracle_backed()
        {
            let primary = primary_quote(context, quotes);
            return Some(scored_near_miss(
                market,
        ...
        let raw_light_profile = raw_light_mode.then(|| {
            codex_scalp_probe_v1_raw_light_profile_for_config(context.target, &self.config)
        });
        ...
        if raw_light_mode {
            let profile = raw_light_profile.unwrap_or_else(|| {
                codex_scalp_probe_v1_raw_light_profile_for_config(context.target, &self.config)
            });
            let target_gap_abs = context.target_gap_bps.abs();
            ...
            let hot_momentum_lane = primary.ask_price > profile.max_entry_price
                ...
                && continuation_coherence;
            let sustained_continuation_lane =
                codex_scalp_probe_v1_raw_light_sustained_continuation_allows(
                    profile,
                    &decision,
                    primary.ask_price,
                    target_gap_abs,
            ...
            if normal_lane || hot_momentum_lane || sustained_continuation_lane {
                let target_notional = self
                    .config
                    .codex_scalp_probe_v1_notional_usdc
                    .min(available_market_capacity)
                    .round_dp(6);
                let executable_cost = build_buy_fill_plan_for_notional(
                    primary_book,
                    target_notional,
                    sweep_average_price_limit(primary.ask_price, max_entry_price),
                )
                .map_or(Decimal::ZERO, |fill| fill.total_cost);
                if !codex_scalp_probe_v1_fill_ratio_allows(
                    executable_cost,
                    target_notional,
                    &self.config,
                ) {
                    let required_cost = (target_notional
                        * self.config.codex_scalp_probe_v1_min_fill_ratio)
                        .round_dp(6);
                    return Some(scored_near_miss(
                        market,
                        context,
                        ...
                        Some(primary.ask_price),
                        None,
                        decimal_to_whole_units(executable_cost),
                        format!(
                            "executable {} < required {}",
                            executable_cost.round_dp(2),
                            required_cost.round_dp(2)
                        ),
                        format!("executable depth is too small for {label}"),
                        0,
                    ));
                }
...
fn codex_scalp_probe_v1_fill_ratio_allows(
    executable_cost: Decimal,
    target_notional: Decimal,
    config: &StrategyConfig,
) -> bool {
    if config.codex_scalp_probe_v1_min_fill_ratio <= Decimal::ZERO {
        return true;
    }

    let required_cost = (target_notional * config.codex_scalp_probe_v1_min_fill_ratio).round_dp(6);
    executable_cost >= required_cost
}
...
fn codex_scalp_probe_v1_raw_light_profile_for_config(
    target: MarketTarget,
    config: &StrategyConfig,
) -> CodexScalpProbeRawLightProfile {
    codex_scalp_probe_v1_scale_raw_light_profile(
        codex_scalp_probe_v1_raw_light_profile(target),
        config.codex_scalp_probe_v1_raw_light_threshold_scale,
    )
}

fn codex_scalp_probe_v1_scale_raw_light_profile(
    mut profile: CodexScalpProbeRawLightProfile,
    scale: Decimal,
) -> CodexScalpProbeRawLightProfile {
    ...
    profile.min_signal_bps = codex_scalp_probe_v1_scaled_threshold(profile.min_signal_bps, scale);
    profile.min_target_gap_bps =
        codex_scalp_probe_v1_scaled_threshold(profile.min_target_gap_bps, scale);
    profile.min_fresh_bps = codex_scalp_probe_v1_scaled_threshold(profile.min_fresh_bps, scale);
    profile.min_aligned_flow_bps =
        codex_scalp_probe_v1_scaled_threshold(profile.min_aligned_flow_bps, scale);
    profile.min_aligned_micro_bps =
        codex_scalp_probe_v1_scaled_threshold(profile.min_aligned_micro_bps, scale);
    profile.min_aligned_swing_bps =
        codex_scalp_probe_v1_scaled_threshold(profile.min_aligned_swing_bps, scale);
    profile.min_top_imbalance_bps =
        codex_scalp_probe_v1_scaled_threshold(profile.min_top_imbalance_bps, scale);
    profile.min_depth_imbalance_bps =
        codex_scalp_probe_v1_scaled_threshold(profile.min_depth_imbalance_bps, scale);
    profile.high_gap_min_signal_bps =
        codex_scalp_probe_v1_scaled_threshold(profile.high_gap_min_signal_bps, scale);
    profile.high_gap_min_aligned_flow_bps =
        codex_scalp_probe_v1_scaled_threshold(profile.high_gap_min_aligned_flow_bps, scale);
    profile.hot_min_signal_bps =
        codex_scalp_probe_v1_scaled_threshold(profile.hot_min_signal_bps, scale);
    profile.hot_min_target_gap_bps =
        codex_scalp_probe_v1_scaled_threshold(profile.hot_min_target_gap_bps, scale);
    profile.hot_min_fresh_bps =
        codex_scalp_probe_v1_scaled_threshold(profile.hot_min_fresh_bps, scale);
    profile.hot_min_aligned_bps =
        codex_scalp_probe_v1_scaled_threshold(profile.hot_min_aligned_bps, scale);
    profile.hot_min_top_imbalance_bps =
        codex_scalp_probe_v1_scaled_threshold(profile.hot_min_top_imbalance_bps, scale);
    profile.hot_min_depth_imbalance_bps =
        codex_scalp_probe_v1_scaled_threshold(profile.hot_min_depth_imbalance_bps, scale);
    profile
}

fn codex_scalp_probe_v1_scaled_threshold(value: Decimal, scale: Decimal) -> Decimal {
    (value * scale).max(Decimal::ZERO).round_dp(6)
}
...
fn codex_scalp_probe_v1_raw_light_allows(
    ...
    let profile = codex_scalp_probe_v1_raw_light_profile_for_config(context.target, config);
    ...
    let hot_momentum_lane = primary_ask_price > profile.max_entry_price
        ...
        && aligned_depth_bps >= profile.hot_min_depth_imbalance_bps
        && continuation_coherence;
    let sustained_continuation_lane = codex_scalp_probe_v1_raw_light_sustained_continuation_allows(
        profile,
        decision,
        primary_ask_price,
        target_gap_abs,
    ...
    normal_lane || hot_momentum_lane || sustained_continuation_lane
}
...
fn codex_scalp_probe_v1_raw_light_sustained_continuation_allows(
    profile: CodexScalpProbeRawLightProfile,
    decision: &BonereaperStateV2Decision,
    primary_ask_price: Decimal,
    target_gap_abs: Decimal,
    aligned_top_bps: Decimal,
    aligned_depth_bps: Decimal,
    continuation_coherence: bool,
) -> bool {
    let sustained_top_threshold = profile
        .min_top_imbalance_bps
        .max(profile.hot_min_top_imbalance_bps / Decimal::from(2));
    let sustained_depth_threshold = profile
        .min_depth_imbalance_bps
        .max(profile.hot_min_depth_imbalance_bps / Decimal::from(2));

    primary_ask_price > profile.max_entry_price
        && primary_ask_price <= profile.hot_max_entry_price
        && decision.signal_strength_bps >= profile.hot_min_signal_bps
        && target_gap_abs >= profile.hot_min_target_gap_bps
        && decision.aligned_micro_bps >= profile.min_aligned_micro_bps
        && decision.aligned_swing_bps >= profile.min_aligned_swing_bps
        && aligned_top_bps >= sustained_top_threshold
        && aligned_depth_bps >= sustained_depth_threshold
        && continuation_coherence
}
...
mod tests {
    ...
    use super::{
        ...
        codex_scalp_probe_v1_allows, codex_scalp_probe_v1_fill_ratio_allows,
        codex_scalp_probe_v1_min_expected_profit_usdc, codex_scalp_probe_v1_raw_light_profile,
        codex_scalp_probe_v1_raw_light_profile_for_config,
        codex_sentinel_v1_aggressive_continuation_allows,
    ...
    fn strategy_config() -> StrategyConfig {
        StrategyConfig {
            ...
            codex_scalp_probe_v1_raw_light_threshold_scale: Decimal::ONE,
            ...
            codex_scalp_probe_v1_min_fill_ratio: Decimal::ZERO,
    ...
    #[test]
    fn codex_scalp_probe_v1_raw_light_threshold_scale_relaxes_profile_thresholds() {
        let mut config = strategy_config();
        config.codex_scalp_probe_v1_raw_light_threshold_scale = decimal("0.75");

        let base = codex_scalp_probe_v1_raw_light_profile(MarketTarget::Btc5m);
        let scaled =
            codex_scalp_probe_v1_raw_light_profile_for_config(MarketTarget::Btc5m, &config);

        assert_eq!(scaled.min_entry_price, base.min_entry_price);
        assert_eq!(scaled.hot_max_entry_price, base.hot_max_entry_price);
        assert_eq!(scaled.max_target_gap_bps, base.max_target_gap_bps);
        assert_eq!(scaled.min_target_gap_bps, decimal("3.000000"));
        assert_eq!(scaled.hot_min_fresh_bps, decimal("5.250000"));
        assert!(scaled.min_signal_bps < base.min_signal_bps);
    }

    #[test]
    fn codex_scalp_probe_v1_fill_ratio_requires_enough_executable_notional() {
        let mut config = strategy_config();
        config.codex_scalp_probe_v1_min_fill_ratio = decimal("0.95");

        assert!(codex_scalp_probe_v1_fill_ratio_allows(
            decimal("28.50"),
            decimal("30.00"),
            &config,
        ));
    ...
    #[test]
    fn codex_scalp_probe_v1_raw_light_allows_sustained_premium_continuation_without_1s() {
        let mut config = strategy_config();
        config.codex_scalp_probe_v1_raw_ablation_enabled = true;
        config.codex_scalp_probe_v1_raw_light_enabled = true;
        config.codex_sentinel_v1_max_live_quote_age_ms = 3_000;
        ...
        let mut context = test_codex_context("7.80", "7.80");
        context.target = MarketTarget::Btc5m;
        context.current_spot_source = "Binance::Trade".to_owned();
        context.current_spot_received_age_ms = Some(25);
        context.exchange_book_age_ms = Some(20);
        context.exchange_book_top_imbalance_bps = Decimal::from(6_200);
        context.exchange_book_depth_imbalance_bps = Decimal::from(4_200);
        context.exchange_book_spread_bps = decimal("0.40");
        context.spot_move_1s_bps = Decimal::ZERO;
        let mut decision = test_codex_decision("7.80", "7.80");
        decision.signal_strength_bps = Decimal::from(3_500);
        decision.aligned_flow_bps = Decimal::from(5_000);
        let book = OrderBook {
            asset_id: "up".to_owned(),
