...
pub fn show_wallet_inventory_replay_report_path(
    ...
    if records.is_empty() {
        info!(path = %path.display(), "wallet-activity replay journal has no records yet");
        return Ok(());
    }
    ...
    info!(
        path = %path.display(),
        records = records.len(),
        "loaded replay inventory report for enriched wallet activity"
    );
    info!(
        "\n{}",
...
pub fn show_wallet_inventory_replay_window_report_path(
    ...
    let Some(window) = window else {
        info!(path = %path.display(), "no suitable replay window found");
        return Ok(());
    };
    ...
    info!(
        ...
        "loaded replay timeline for enriched wallet activity"
    );
    info!(
        "\n{}",
...
pub fn export_wallet_inventory_replay_dataset_path(
    ...
    if records.is_empty() {
        info!(path = %input_path.display(), "wallet-activity replay journal has no records yet");
        return Ok(());
    }
    ...
    info!(
        ...
        "exported replay dataset with inventory alerts"
    );
    Ok(())
}
...
pub fn show_wallet_inventory_replay_simulation_path(
    ...
    info!(
        ...
        "loaded replay cap/cooldown simulator"
    );
    info!(
        "\n{}",
...
fn render_wallet_inventory_replay_report(
    ...
    let _ = writeln!(output, "Windows: {}", summaries.len());
    let _ = writeln!(output, "TRADE events: {trade_events}");
    let _ = writeln!(output, "REDEEM events: {redeem_events}");
    ...
    let _ = writeln!(output, "Windows with redeem: {windows_with_redeem}");
    let _ = writeln!(output, "Two-sided windows at exit: {two_sided_windows}");
    let _ = writeln!(output, "Balanced windows at exit: {balanced_windows}");
    let _ = writeln!(
        output,
    ...
    let _ = writeln!(
        output,
        "Hedged share of final inventory: {}",
...
fn render_inventory_replay_simulation_report(
    ...
    let _ = writeln!(output, "Windows: {}", summary.windows);
...
fn render_wallet_inventory_window_report(
    ...
    let _ = writeln!(output, "TRADE events: {}", window.trade_events());
    let _ = writeln!(output, "REDEEM events: {}", window.redeem_events());
    let _ = writeln!(
        output,
        "Trade volume USDC: {}",
