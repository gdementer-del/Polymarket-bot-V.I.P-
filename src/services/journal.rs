...
pub struct PaperCycleEntry {
    ...
    #[serde(default)]
    pub processed_cycle_count: usize,
...
const PAPER_JOURNAL_ACK_TIMEOUT_SECS: u64 = 5;
...
#[derive(Default)]
struct PaperJournalBatch {
    commands: Vec<PaperJournalBatchCommand>,
    latest_cycle: Option<Box<PaperCycleEntry>>,
    shutdown_seen: bool,
}
...
impl PaperJournalWriter {
    ...
    pub fn flush(&self) -> Result<()> {
        ...
        ack_rx
            .recv_timeout(paper_journal_ack_timeout())
            .map_err(writer_recv_timeout_error)?
    ...
    fn shutdown_inner(&mut self) -> Result<()> {
        ...
        ack_rx
            ...
            .map_err(|error| {
                ...
                writer_recv_timeout_error(error)
            })?
            .map_err(|error| writer_persist_error(&error))?;
...
impl Drop for PaperJournalWriter {
    fn drop(&mut self) {
        ...
        if let Err(error) = self.shutdown_inner() {
            warn!(
                error = %error,
                "failed to shut down the paper-journal background writer cleanly"
            );
            if let Some(handle) = self.handle.take() {
                let _ = handle.join();
            }
...
fn collect_paper_journal_batch(
    first: PaperJournalCommand,
    rx: &StdReceiver<PaperJournalCommand>,
) -> PaperJournalBatch {
    let mut batch = PaperJournalBatch::default();
    push_paper_journal_batch_command(&mut batch, first);

    while !batch.shutdown_seen {
        match rx.try_recv() {
            Ok(command) => push_paper_journal_batch_command(&mut batch, command),
...
fn push_paper_journal_batch_command(batch: &mut PaperJournalBatch, command: PaperJournalCommand) {
    match command {
        PaperJournalCommand::Execution { snapshot, entry } => {
            batch
                .commands
                .push(PaperJournalBatchCommand::Execution { snapshot, entry });
        }
        PaperJournalCommand::Snapshot(snapshot) => {
            batch
                .commands
                .push(PaperJournalBatchCommand::Snapshot(snapshot));
        }
        PaperJournalCommand::Trade(entry) => {
            batch.commands.push(PaperJournalBatchCommand::Trade(entry));
        }
        PaperJournalCommand::Cycle(entry) => {
            batch.commands.push(PaperJournalBatchCommand::Cycle(entry));
        }
        PaperJournalCommand::CycleLatest(entry) => {
            batch.latest_cycle = Some(entry);
        }
        PaperJournalCommand::Flush(ack_tx) => {
            batch.commands.push(PaperJournalBatchCommand::Flush(ack_tx));
        }
        PaperJournalCommand::Shutdown(ack_tx) => {
            batch.shutdown_seen = true;
            batch
                .commands
                .push(PaperJournalBatchCommand::Shutdown(ack_tx));
        }
    }
}

fn persist_paper_journal_batch(
    store: &JournalStore,
    mut batch: PaperJournalBatch,
    last_error: &mut Option<String>,
) -> bool {
    let mut shutdown = false;
    let mut pending_latest_cycle = batch.latest_cycle.take();

    for command in batch.commands {
        match command {
            PaperJournalBatchCommand::Execution { snapshot, entry } => {
                record_paper_writer_result(
                    store.persist_execution(&snapshot, &entry),
                    last_error,
                    "persist_execution",
                );
            }
            PaperJournalBatchCommand::Snapshot(snapshot) => {
                record_paper_writer_result(
            ...
            PaperJournalBatchCommand::Trade(entry) => {
                record_paper_writer_result(
                    store.record_paper_trade(&entry),
                    last_error,
                    "record_paper_trade",
                );
            }
            PaperJournalBatchCommand::Cycle(entry) => {
                record_paper_writer_result(
                    store.record_paper_cycle(&entry),
                    last_error,
                    "record_paper_cycle",
                );
            }
            PaperJournalBatchCommand::Flush(ack_tx) => {
                persist_pending_latest_cycle(store, &mut pending_latest_cycle, last_error);
                let _ = ack_tx.send(paper_writer_ack(last_error.as_deref()));
            }
            PaperJournalBatchCommand::Shutdown(ack_tx) => {
                persist_pending_latest_cycle(store, &mut pending_latest_cycle, last_error);
                let _ = ack_tx.send(paper_writer_ack(last_error.as_deref()));
                shutdown = true;
            }
...
fn persist_pending_latest_cycle(
    store: &JournalStore,
    pending_latest_cycle: &mut Option<Box<PaperCycleEntry>>,
    last_error: &mut Option<String>,
) {
    if let Some(entry) = pending_latest_cycle.take() {
        record_paper_writer_result(
            store.record_paper_cycle_latest(&entry),
            last_error,
            "record_paper_cycle_latest",
        );
    }
}

fn record_paper_writer_result(
    result: Result<()>,
    last_error: &mut Option<String>,
    ...
    if let Err(error) = result {
        ...
        warn!(
            error = %error,
            operation,
            "failed to persist paper journal command asynchronously"
        );
    }
}
...
impl JournalStore {
    ...
    pub fn spawn_paper_writer(&self) -> PaperJournalWriter {
        ...
        let handle = thread::spawn(move || {
            ...
            while let Ok(command) = rx.recv() {
                let batch = collect_paper_journal_batch(command, &rx);
                if persist_paper_journal_batch(&store, batch, &mut last_error) {
                    break;
                }
            }
        });
    ...
    pub fn load_paper_cycles(&self, limit: Option<usize>) -> Result<Vec<PaperCycleEntry>> {
        let mut entries = Vec::<PaperCycleEntry>::new();
        for path in self.paper_cycle_journal_paths()? {
            entries.extend(load_jsonl_records(&path, None)?);
        }
        if let Some(latest) = self.load_latest_paper_cycle()?
            && entries
                .last()
                .is_none_or(|entry| latest.recorded_at > entry.recorded_at)
        {
            entries.push(latest);
        }
...
fn writer_recv_timeout_error(error: mpsc::RecvTimeoutError) -> AppError {
    match error {
        mpsc::RecvTimeoutError::Timeout => AppError::Io(std::io::Error::new(
            std::io::ErrorKind::TimedOut,
            "paper journal writer did not acknowledge within timeout",
        )),
        mpsc::RecvTimeoutError::Disconnected => AppError::Io(std::io::Error::new(
            std::io::ErrorKind::BrokenPipe,
            "paper journal writer disconnected before acknowledging",
        )),
    }
}
...
fn paper_journal_ack_timeout() -> StdDuration {
    StdDuration::from_secs(PAPER_JOURNAL_ACK_TIMEOUT_SECS)
}
...
mod tests {
    ...
    use super::{
        ...
        PaperReportMemory, PaperTradeAction, PaperTradeEntry, PnlSnapshot,
    };
    ...
    fn build_paper_trade_entry(position_id: &str) -> PaperTradeEntry {
        PaperTradeEntry {
            run_id: "paper-test-run".to_owned(),
            ...
            question: "Will BTC finish higher in 5m?".to_owned(),
            kind: OpportunityKind::BundleArbitrage,
            spent_usdc: Decimal::new(30, 0),
            expected_profit_usdc: Some(Decimal::new(3, 0)),
            realized_payout_usdc: None,
            realized_profit_usdc: None,
            dominant_outcome: Some("Up".to_owned()),
            actual_outcome: None,
            holding_seconds: None,
            close_category: None,
            current_spot_price: Some(Decimal::from(67_050_u32)),
            target_price: Some(Decimal::from(67_000_u32)),
            target_gap_bps: Some(Decimal::new(75, 1)),
            spot_move_bps: Some(Decimal::new(75, 1)),
            spot_move_1s_bps: Some(Decimal::ZERO),
            spot_move_5s_bps: Some(Decimal::ZERO),
            spot_move_15s_bps: Some(Decimal::ZERO),
            micro_acceleration_bps: Some(Decimal::ZERO),
            signal_strength_bps: Some(Decimal::ZERO),
            aligned_trade_flow_bps: Some(Decimal::ZERO),
            primary_outcome_ask_price: Some(Decimal::new(46, 2)),
            signal_tier: Some("test".to_owned()),
            target_cross_label: Some("none".to_owned()),
            seconds_left_at_entry: Some(120),
            note: "test".to_owned(),
        }
    }

    fn build_paper_cycle_entry(run_id: &str, processed_cycle_count: usize) -> PaperCycleEntry {
        PaperCycleEntry {
            ...
            top_opportunity_slug: None,
            top_opportunity_kind: None,
            top_opportunity_edge_bps: None,
            top_opportunity_required_usdc: None,
            top_opportunity_expected_profit_usdc: None,
            top_opportunity_signal_strength_bps: None,
            top_opportunity_target_gap_bps: None,
            top_opportunity_primary_ask: None,
            top_opportunity_signal_tier: None,
            top_opportunity_target_cross_label: None,
            top_near_miss_slug: None,
            top_near_miss_reason: None,
            top_near_miss_primary_ask: None,
            top_near_miss_bundle_cost: None,
            top_near_miss_target_gap_bps: None,
            top_near_miss_spot_move_bps: None,
            top_near_miss_spot_move_1s_bps: None,
            top_near_miss_spot_move_5s_bps: None,
            top_near_miss_spot_move_15s_bps: None,
            top_near_miss_micro_acceleration_bps: None,
            top_near_miss_exchange_book_age_ms: None,
            top_near_miss_exchange_book_top_imbalance_bps: None,
            top_near_miss_exchange_book_depth_imbalance_bps: None,
            top_near_miss_shortfall_bps: None,
            top_near_miss_shortfall_label: None,
            current_market_slug: None,
            current_market_seconds_left: None,
            current_market_spot_move_bps: None,
            current_market_spot_move_1s_bps: None,
            current_market_spot_move_5s_bps: None,
            current_market_spot_move_15s_bps: None,
            current_market_micro_acceleration_bps: None,
            current_market_price: None,
            current_market_spot_source: None,
            current_market_spot_event_age_ms: None,
            current_market_spot_received_age_ms: None,
            current_market_spot_quote_points: None,
            current_market_exchange_book_age_ms: None,
            current_market_exchange_book_top_imbalance_bps: None,
            current_market_exchange_book_depth_imbalance_bps: None,
            current_market_exchange_book_microprice_bps: None,
            current_market_exchange_book_spread_bps: None,
            current_market_target_price: None,
            current_market_target_price_source: None,
            current_market_target_gap_bps: None,
            current_market_up_ask: None,
            current_market_down_ask: None,
            current_market_bundle_cost: None,
            current_market_direction: None,
            current_market_fit: None,
            ...
            risk_blocked: false,
            risk_reason: None,
            daily_realized_profit: Decimal::ZERO,
            session_realized_profit: Decimal::ZERO,
            consecutive_losses: 0,
            worst_open_slug: None,
            worst_open_mtm_profit_usdc: None,
            worst_open_stop_loss_hit: None,
            worst_open_aligned_1s_bps: None,
            worst_open_aligned_5s_bps: None,
            worst_open_aligned_15s_bps: None,
            latency: PaperCycleLatencyMetrics::default(),
        }
    }

    #[test]
    fn paper_writer_batch_coalesces_latest_cycles_but_keeps_trades() {
        let (tx, rx) = mpsc::channel();
        let (ack_tx, _ack_rx) = mpsc::channel();

        tx.send(super::PaperJournalCommand::CycleLatest(Box::new(
            build_paper_cycle_entry("stale-cycle", 1),
        )))
        .expect("first latest cycle should queue");
        tx.send(super::PaperJournalCommand::CycleLatest(Box::new(
            build_paper_cycle_entry("fresh-cycle", 2),
        )))
        .expect("second latest cycle should queue");
        tx.send(super::PaperJournalCommand::Trade(Box::new(
            build_paper_trade_entry("paper-pos-1"),
        )))
        .expect("trade should queue");
        tx.send(super::PaperJournalCommand::Flush(ack_tx))
            .expect("flush should queue");

        let first = rx.recv().expect("first command should be readable");
        let batch = super::collect_paper_journal_batch(first, &rx);
        let latest = batch
            .latest_cycle
            .as_ref()
            .expect("latest cycle should be retained");

        assert_eq!(latest.run_id, "fresh-cycle");
        assert_eq!(latest.processed_cycle_count, 2);
        assert_eq!(batch.commands.len(), 2);
        assert!(matches!(
            batch.commands[0],
            super::PaperJournalBatchCommand::Trade(_)
        ));
        assert!(matches!(
            batch.commands[1],
            super::PaperJournalBatchCommand::Flush(_)
        ));
    }
    ...
    fn journal_persists_and_reads_paper_cycles() {
        ...
        let entry = PaperCycleEntry {
            ...
            top_near_miss_reason: Some("spot move is below threshold".to_owned()),
            ...
            current_market_direction: Some("Up".to_owned()),
            current_market_fit: Some(false),
            decision_reason: Some("near directional | gap 2 bps".to_owned()),
            current_market_health: PaperCycleCurrentMarketHealth::default(),
        ...
        let mut latest_only = second_entry.clone();
        latest_only.run_id = "test-run-3".to_owned();
        latest_only.recorded_at += chrono::Duration::seconds(1);
        latest_only.opportunity_count = 3;
        store
            .record_paper_cycle_latest(&latest_only)
            .expect("latest-only paper cycle write should succeed");
        let loaded_with_latest = store
            .load_paper_cycles(Some(10))
            .expect("latest-only cycle should be included in paper cycle reads");
        assert_eq!(loaded_with_latest.len(), 3);
        assert_eq!(loaded_with_latest[2].run_id, "test-run-3");
        assert_eq!(loaded_with_latest[2].opportunity_count, 3);
