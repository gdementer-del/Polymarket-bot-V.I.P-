...
const STREAM_RECONNECT_MAX_DELAY_MS: u64 = 5_000;
const RTDS_PING_INTERVAL_MS: u64 = 5_000;
const RTDS_SNAPSHOT_REFRESH_INTERVAL_MS: i64 = 15_000;
...
impl ChainlinkOracleCache {
    ...
    pub async fn decorate_context(
        ...
        let (target_price, target_price_source) = if let Some(target_price) = market.target_price {
            ...
            .live_window_open_quote(
                target,
                symbol,
                start_ts,
                settings.max_live_context_open_lag_ms,
            )
            .await
        {
            (open_quote.price, TargetPriceSource::ChainlinkRtdsWindowOpen)
        } else if let Some(target_price) = basis_adjusted_chainlink_target_price(
            context.interval_open_price,
            context.current_spot_price,
            latest.price,
        ) {
            (target_price, TargetPriceSource::ChainlinkRtdsBasisAdjusted)
        } else {
            return false;
        };
    ...
    async fn run_stream_loop(self, websocket_url: String, symbols: Vec<String>) {
        let configured_symbols = symbols.iter().cloned().collect::<HashSet<_>>();
        let mut reconnect_backoff =
            ReconnectBackoff::new(STREAM_RECONNECT_DELAY_MS, STREAM_RECONNECT_MAX_DELAY_MS);
        loop {
            match connect_async(&websocket_url).await {
                Ok((stream, _response)) => {
                    ...
                    if let Err(error) = subscribe_chainlink_stream(&mut writer, &symbols).await {
                        warn!(
                            error = %error,
                    ...
                    let mut maintenance = interval(Duration::from_millis(RTDS_PING_INTERVAL_MS));
                    let mut next_snapshot_refresh_ms = Utc::now()
                        .timestamp_millis()
                        .saturating_add(RTDS_SNAPSHOT_REFRESH_INTERVAL_MS);
                    loop {
                        tokio::select! {
                            _ = maintenance.tick() => {
                                ...
                                let now_ms = Utc::now().timestamp_millis();
                                if now_ms >= next_snapshot_refresh_ms {
                                    if let Err(error) = subscribe_chainlink_snapshots(&mut writer, &symbols).await {
                                        warn!(
                                            error = %error,
                                            "failed to refresh Polymarket RTDS Chainlink snapshots"
                                        );
                                        break;
                                    }
                                    next_snapshot_refresh_ms = now_ms
                                        .saturating_add(RTDS_SNAPSHOT_REFRESH_INTERVAL_MS);
                                }
                            }
                            next_message = reader.next() => {
                                match next_message {
                                    Some(Ok(Message::Text(payload))) => {
                                        if let Err(error) = self.handle_text_message_for_symbols(
                                            payload.as_ref(),
                                            Some(&configured_symbols),
                                        ).await {
                                            warn!(
                                                error = %error,
    ...
    async fn handle_text_message(&self, payload: &str) -> Result<()> {
        self.handle_text_message_for_symbols(payload, None).await
    }

    async fn handle_text_message_for_symbols(
        &self,
        payload: &str,
        configured_symbols: Option<&HashSet<String>>,
    ) -> Result<()> {
        ...
        match message.topic.as_str() {
            "crypto_prices_chainlink" => {
                self.handle_update_message(message, configured_symbols)
                    .await
            }
            "crypto_prices" => {
                self.handle_snapshot_message(message, configured_symbols)
                    .await
            }
            _ => Ok(()),
        }
    ...
    async fn handle_update_message(
        ...
        if !chainlink_symbol_is_configured(configured_symbols, &symbol) {
            return Ok(());
        }
    ...
    async fn handle_snapshot_message(
        &self,
        message: RtdsEnvelope,
        configured_symbols: Option<&HashSet<String>>,
    ) -> Result<()> {
        ...
        let symbol = normalize_chainlink_symbol(&payload.symbol);
        if symbol.is_empty() || !chainlink_symbol_is_configured(configured_symbols, &symbol) {
            return Ok(());
        }

        let received_time_ms = Utc::now().timestamp_millis();
        let mut quotes = Vec::with_capacity(payload.data.len());
        ...
        self.ingest_snapshot_quotes(&symbol, &quotes).await;
    ...
    async fn ingest_snapshot_quotes(&self, symbol: &str, quotes: &[ChainlinkOracleQuote]) {
        if quotes.is_empty() {
            return;
        }
        ...
        let mut state = self.state.write().await;
        {
            let cached_quotes = state
                .quotes_by_symbol
                .entry(symbol.clone())
                .or_insert_with(VecDeque::new);
            for quote in quotes {
                insert_chainlink_quote_ordered(cached_quotes, *quote);
            }
            if let Some(latest_event_time_ms) = quotes.iter().map(|quote| quote.event_time_ms).max()
            {
                prune_quote_history(cached_quotes, latest_event_time_ms);
            }
        }
        ...
        debug!(
            symbol = %symbol,
            points = quotes.len(),
            latest_event_time_ms = quotes.iter().map(|quote| quote.event_time_ms).max(),
            "seeded Chainlink RTDS oracle cache from snapshot"
        );
    }
    ...
    async fn live_window_open_quote(
        &self,
        target: MarketTarget,
        symbol: &str,
        start_ts: i64,
        ...
        if let Some(quote) = self
            .nearest_quote_to_window_open(symbol, start_ts, max_open_lag_ms)
            .await
        {
            return Some(quote);
        }
        ...
        if let Some(quote) = self
            .window_open_quote(target, start_ts, max_open_lag_ms)
            .await
        {
            return Some(quote);
        }
    ...
    async fn nearest_quote_to_window_open(
        &self,
        symbol: &str,
        start_ts: i64,
        max_open_lag_ms: i64,
    ) -> Option<ChainlinkOracleQuote> {
        let max_open_lag_ms = u64::try_from(max_open_lag_ms).ok()?;
        let start_ms = start_ts.saturating_mul(1_000);
        let normalized_symbol = normalize_chainlink_symbol(symbol);
        let state = self.state.read().await;
        state
            .quotes_by_symbol
            ...
            .copied()
            .filter_map(|quote| {
                let lag_ms = quote.event_time_ms.abs_diff(start_ms);
                (lag_ms <= max_open_lag_ms).then_some((lag_ms, quote))
            })
            .min_by_key(|(lag_ms, quote)| {
                (*lag_ms, quote.event_time_ms < start_ms, quote.event_time_ms)
            })
            .map(|(_lag_ms, quote)| quote)
    }
...
fn chainlink_subscribe_all_live_message() -> serde_json::Value {
    serde_json::json!({
        "action": "subscribe",
        "subscriptions": [{
            "topic": "crypto_prices_chainlink",
...
async fn subscribe_chainlink_stream(
    ...
    // Specific subscriptions seed each configured symbol with its startup snapshot.
    subscribe_chainlink_snapshots(writer, symbols).await?;
    // RTDS currently emits multi-asset live ticks reliably through the all-symbol form.
    // Incoming updates are filtered against the configured allowlist before ingestion.
    writer
        .send(Message::Text(
            chainlink_subscribe_all_live_message().to_string().into(),
        ))
        .await
}

async fn subscribe_chainlink_snapshots(
    writer: &mut futures_util::stream::SplitSink<
        tokio_tungstenite::WebSocketStream<
            tokio_tungstenite::MaybeTlsStream<tokio::net::TcpStream>,
        >,
    ...
    writer
        .send(Message::Text(
            chainlink_subscribe_message(symbols).to_string().into(),
        ))
        .await
}

fn chainlink_symbol_is_configured(
    configured_symbols: Option<&HashSet<String>>,
    symbol: &str,
) -> bool {
    configured_symbols.is_none_or(|symbols| symbols.contains(symbol))
}
...
fn basis_adjusted_chainlink_target_price(
    exchange_open_price: Decimal,
    exchange_current_price: Decimal,
    chainlink_current_price: Decimal,
) -> Option<Decimal> {
    if exchange_open_price <= Decimal::ZERO
        || exchange_current_price <= Decimal::ZERO
        || chainlink_current_price <= Decimal::ZERO
    {
        return None;
    }

    Some((exchange_open_price * chainlink_current_price / exchange_current_price).round_dp(4))
}
...
mod tests {
    ...
    use super::{ChainlinkOracleCache, chainlink_subscribe_all_live_message};
    ...
    fn test_settings() -> ChainlinkOracleConfig {
        ChainlinkOracleConfig {
            ...
            max_live_context_open_lag_ms: 30_000,
    ...
    #[tokio::test]
    async fn decorate_context_accepts_late_open_quote_with_live_context_lag() {
        let cache = ChainlinkOracleCache::default();
        let mut settings = test_settings();
        settings.max_window_open_lag_ms = 3_000;
        settings.max_live_context_open_lag_ms = 30_000;
        cache.set_test_settings(settings).await;
        cache
            .ingest_test_quote("btc/usd", 925_000, decimal("100"))
            .await;
        cache
            .ingest_test_quote("btc/usd", 926_000, decimal("101"))
            .await;
        let mut market = test_market();
        market.target_price = None;
        market.target_price_source = None;
        let mut context = test_context();

        assert!(cache.decorate_context(&market, &mut context).await);

        assert_eq!(context.target_price, decimal("100"));
        assert_eq!(
            context.target_price_source,
            TargetPriceSource::ChainlinkRtdsWindowOpen
        );
        assert_eq!(context.current_spot_price, decimal("101"));
    }

    #[tokio::test]
    async fn decorate_context_prefers_nearest_pre_open_quote_for_live_anchor() {
        let cache = ChainlinkOracleCache::default();
        cache.set_test_settings(test_settings()).await;
        cache
            .ingest_test_quote("btc/usd", 898_000, decimal("100"))
            .await;
        cache
            .ingest_test_quote("btc/usd", 910_000, decimal("102"))
            .await;
        cache
            .ingest_test_quote("btc/usd", 912_000, decimal("103"))
            .await;
        let mut market = test_market();
        market.target_price = None;
        market.target_price_source = None;
        ...
        assert_eq!(context.target_price, decimal("100"));
        assert_eq!(context.current_spot_price, decimal("103"));
        assert_eq!(context.target_gap_bps, decimal("300.0000"));
        assert_eq!(
            context.target_price_source,
            TargetPriceSource::ChainlinkRtdsWindowOpen
        );
    ...
    #[tokio::test]
    async fn decorate_context_uses_basis_adjusted_target_when_open_quote_is_missing() {
        let cache = ChainlinkOracleCache::default();
        cache.set_test_settings(test_settings()).await;
        cache
            .ingest_test_quote("btc/usd", 960_000, decimal("102"))
            .await;
        let mut market = test_market();
        market.target_price = None;
        market.target_price_source = None;
        let mut context = test_context();
        context.interval_open_price = decimal("100");
        context.target_price = decimal("100");
        context.current_spot_price = decimal("101");

        assert!(cache.decorate_context(&market, &mut context).await);

        assert_eq!(context.target_price, decimal("100.9901"));
        assert_eq!(context.current_spot_price, decimal("102"));
        assert_eq!(
            context.target_price_source,
            TargetPriceSource::ChainlinkRtdsBasisAdjusted
        );
    }

    #[tokio::test]
    async fn snapshot_message_emits_only_latest_quote_trigger() {
        let cache = ChainlinkOracleCache::default();
        let mut rx = cache.subscribe_triggers();
        cache
            .handle_text_message(
                r#"{"payload":{"data":[{"timestamp":902000,"value":101.2},{"timestamp":901000,"value":101.1}],"symbol":"btc/usd"},"timestamp":902500,"topic":"crypto_prices","type":"subscribe"}"#,
            )
            .await
            .expect("snapshot parses");

        rx.changed().await.expect("snapshot emits one trigger");
        let trigger = rx.borrow_and_update().clone().expect("latest trigger");

        assert_eq!(trigger.symbol, "btc/usd");
        assert_eq!(trigger.event_time_ms, 902_000);
        assert_eq!(trigger.price, decimal("101.2"));
        assert!(!rx.has_changed().expect("trigger channel remains available"));
    }

    #[test]
    fn all_symbol_chainlink_subscription_uses_one_empty_filter() {
        let message = chainlink_subscribe_all_live_message();
        let subscriptions = message["subscriptions"]
            .as_array()
            .expect("subscriptions array");

        assert_eq!(subscriptions.len(), 1);
        assert_eq!(subscriptions[0]["topic"], "crypto_prices_chainlink");
        assert_eq!(subscriptions[0]["type"], "*");
        assert_eq!(subscriptions[0]["filters"], "");
    }

    #[tokio::test]
    async fn live_update_outside_configured_symbols_is_ignored() {
        let cache = ChainlinkOracleCache::default();
        let rx = cache.subscribe_triggers();
        let configured_symbols = HashSet::from(["btc/usd".to_owned()]);

        cache
            .handle_text_message_for_symbols(
                r#"{"payload":{"symbol":"doge/usd","timestamp":902000,"value":0.1},"timestamp":902000,"topic":"crypto_prices_chainlink","type":"update"}"#,
                Some(&configured_symbols),
            )
            .await
            .expect("unconfigured update is ignored");
        ...
        assert!(
            !cache
                ...
                .await
                .quotes_by_symbol
                .contains_key("doge/usd")
        );
    }

    #[tokio::test]
    async fn live_update_inside_configured_symbols_is_ingested() {
        let cache = ChainlinkOracleCache::default();
        let mut rx = cache.subscribe_triggers();
        let configured_symbols = HashSet::from(["btc/usd".to_owned()]);
        ...
        cache
            .handle_text_message_for_symbols(
                r#"{"payload":{"symbol":"btc/usd","timestamp":902000,"value":101.2},"timestamp":902000,"topic":"crypto_prices_chainlink","type":"update"}"#,
                Some(&configured_symbols),
            )
            .await
            .expect("configured update parses");

        rx.changed().await.expect("configured update emits trigger");
        let trigger = rx.borrow_and_update().clone().expect("latest trigger");
    ...
    #[tokio::test]
    async fn subscribe_ack_without_topic_is_ignored() {
        let cache = ChainlinkOracleCache::default();
        cache.set_test_settings(test_settings()).await;

        cache
            .handle_text_message(r#"{"status":"success","message":"subscribed"}"#)
            .await
            .expect("control frames without a topic are ignored");
    }
