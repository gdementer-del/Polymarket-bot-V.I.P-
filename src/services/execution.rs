...
impl LiveCredentialsMode {
    const fn as_str(self) -> &'static str {
        match self {
            Self::Provided => "provided manually",
            Self::Derived => "derived automatically",
        }
    }
}
...
impl LiveFunderMode {
    const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::Explicit => "provided explicitly",
            Self::AutoDerived => "derived automatically",
        }
    }
}
...
fn resolve_live_auth(live_config: &LiveConfig) -> Result<ResolvedLiveAuth> {
    ...
    let private_key = resolve_required_secret(
        ...
        "Polymarket private key (required to sign live orders): ",
    )?;
        ...
        .filter(|value| !value.is_empty())
    {
        Some(value) => Some(Address::from_str(value).map_err(|error| {
            AppError::InvalidLiveAuth(format!("invalid funder address: {error}"))
        })?),
        None => None,
    };
...
fn resolve_optional_credentials(live_config: &LiveConfig) -> Result<ResolvedCredentials> {
        ...
        && live_config.prompt_for_secrets
    {
        let input = prompt_line(
            "Polymarket API key (optional; press Enter to derive it from the private key): ",
    ...
    match (api_key, api_secret, api_passphrase) {
        (Some(api_key), Some(api_secret), Some(api_passphrase)) => {
            let api_key = ApiKey::from_str(&api_key).map_err(|error| {
                AppError::InvalidLiveAuth(format!("invalid API-key UUID: {error}"))
            })?;
    ...
    async fn paper_close_position_realizes_profit() {
        ...
        let close_report = executor
            .close_position(
                &opportunity.slug,
                &resolution,
                "window ended; position settled against the resolved outcome",
            )
            .await
