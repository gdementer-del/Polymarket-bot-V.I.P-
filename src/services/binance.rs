...
struct LiveQuoteReferenceSnapshot {
    ...
    latest_momentum_quote: Option<LiveSpotQuote>,
...
impl BinanceClient {
    ...
    pub fn start_trade_stream(&self, symbol: &str) -> bool {
...

        {
            let Ok(mut started) = self.started_symbols.lock() else {
                warn!("started_symbols mutex poisoned; skipping Binance stream startup");
                return false;
            };
    ...
    async fn run_trade_stream(self, symbol: String, stream_url: String) {
        ...
        loop {
            match connect_async(&stream_url).await {
                Ok((stream, _response)) => {
                    reconnect_backoff.reset();
                    info!(symbol = %symbol, "connected Binance trade stream");
                    let (_writer, mut reader) = stream.split();
                            ...
                            Ok(Message::Text(payload)) => {
                                    ...
                                    self.update_trade_quote(&symbol, payload.as_ref()).await
                                {
                                    warn!(
                                        symbol = %symbol,
                                        error = %error,
                                        "failed to update cached Binance quote"
                                    );
                                }
                            }
                            Ok(Message::Close(frame)) => {
                                info!(symbol = %symbol, ?frame, "Binance trade stream closed");
                                break;
                            }
                            Ok(_) => {}
                            Err(error) => {
                ...
                Err(error) => {
                    warn!(
                        symbol = %symbol,
                        error = %error,
                        "failed to connect to Binance trade stream"
                    );
                }
            }
    ...
    async fn run_second_kline_stream(self, symbol: String, stream_url: String) {
        ...
        loop {
            match connect_async(&stream_url).await {
                Ok((stream, _response)) => {
                    reconnect_backoff.reset();
                    info!(symbol = %symbol, "connected Binance 1s kline stream");
                    let (_writer, mut reader) = stream.split();
                            ...
                            Ok(Message::Text(payload)) => {
                                    ...
                                    self.update_second_kline(&symbol, payload.as_ref()).await
                                {
                                    warn!(
                                        symbol = %symbol,
                                        error = %error,
                                        "failed to update Binance 1s kline cache"
                                    );
                                }
                            }
                            Ok(Message::Close(frame)) => {
                                info!(symbol = %symbol, ?frame, "Binance 1s kline stream closed");
                                break;
                            }
                            Ok(_) => {}
                            Err(error) => {
                ...
                Err(error) => {
                    warn!(
                        symbol = %symbol,
                        error = %error,
                        "failed to connect to Binance 1s kline stream"
                    );
                }
            }
    ...
    async fn live_signal_state_from_streams(
        ...
        let momentum_spot_price = quote_snapshot
            .latest_momentum_quote
            .map_or(current_spot_price, |quote| quote.price);
        let micro_burst_reference_price = quote_snapshot
            .micro_burst_reference_price
            .unwrap_or(momentum_spot_price);
    ...
    async fn live_price_snapshot(&self, symbol: &str) -> Result<LivePriceSnapshot> {
        ...
        let spot_move_1s_bps = if micro_burst_reference_price == Decimal::ZERO {
            ...
            ((momentum_spot_price - micro_burst_reference_price) / micro_burst_reference_price
                * Decimal::from(10_000_u32))
            .round_dp(4)
        };
        let spot_move_5s_bps = if micro_reference_price == Decimal::ZERO {
            ...
            ((momentum_spot_price - micro_reference_price) / micro_reference_price
                * Decimal::from(10_000_u32))
            .round_dp(4)
        };
        let momentum_move_bps = if momentum_reference_price == Decimal::ZERO {
            ...
            ((momentum_spot_price - momentum_reference_price) / momentum_reference_price
                * Decimal::from(10_000_u32))
            .round_dp(4)
        };
        let micro_acceleration_bps =
        ...
        if spot_move_5s_bps == Decimal::ZERO {
            info!(
                ...
                momentum_spot_price = %momentum_spot_price,
    ...
    async fn live_quote_reference_snapshot(
        ...
        let latest_momentum_quote = latest_fresh_quote_from_source(
            quotes,
            SpotQuoteSource::BinanceTrade,
            now_ms,
            MAX_CACHED_QUOTE_AGE_MS,
        );
        let micro_reference_price = latest_momentum_quote.and_then(|latest_quote| {
            reference_quote_for_window_from_source(
                quotes,
        ...
        let micro_burst_reference_price = latest_momentum_quote.and_then(|latest_quote| {
            reference_quote_for_window_from_source(
                quotes,
                latest_quote.event_time_ms,
                MICRO_BURST_PRICE_WINDOW_MS,
        ...
        let momentum_reference_price = latest_momentum_quote.and_then(|latest_quote| {
            reference_quote_for_window_from_source(
                quotes,
                latest_quote.event_time_ms,
                MOMENTUM_PRICE_WINDOW_MS,
    ...
    async fn one_minute_open(&self, symbol: &str, start_time_ms: i64) -> Result<Decimal> {
        ...
        let Some(first_kline) = klines.first() else {
            return Err(AppError::InvalidMarket(
                "Binance did not return a kline for the window start".to_owned(),
            ));
        };
        ...
        klines
            ...
            .ok_or_else(|| {
                AppError::InvalidMarket(
                    "Binance did not return a kline for the historical window timestamp".to_owned(),
                )
            })
    }
    ...
    async fn reference_price_from_stream(&self, symbol: &str, window_ms: i64) -> Result<Decimal> {
        ...
        let Some(klines) = snapshot else {
            return Err(AppError::InvalidMarket(
                "Binance has not accumulated 1s klines for the 5-second move yet".to_owned(),
            ));
        };
        ...
        reference_kline_open_price_for_window(&klines, latest_kline.open_time_ms, window_ms)
            .ok_or_else(|| {
                AppError::InvalidMarket(
                    "Binance has not accumulated enough 1s klines for the 5-second move yet"
                        .to_owned(),
                )
            })
    }
    ...
    async fn reference_price_from_rest(&self, symbol: &str, window_ms: i64) -> Result<Decimal> {
        ...
        let Some(reference_kline) = klines.get(klines.len().saturating_sub(offset + 1)) else {
            return Err(AppError::InvalidMarket(
                "Binance did not return enough 1s klines for the 5-second move".to_owned(),
            ));
        };
...
fn build_live_price_snapshot(
    ...
    quote_points: usize,
    exchange_book_pressure: Option<ExchangeBookPressureSnapshot>,
    momentum_spot_price: Decimal,
    micro_burst_reference_price: Decimal,
    micro_reference_price: Decimal,
    momentum_reference_price: Decimal,
) -> LivePriceSnapshot {
    let spot_move_1s_bps = historical_move_bps(momentum_spot_price, micro_burst_reference_price);
    let spot_move_5s_bps = historical_move_bps(momentum_spot_price, micro_reference_price);
    let momentum_move_bps = historical_move_bps(momentum_spot_price, momentum_reference_price);
    let micro_acceleration_bps =
        (spot_move_5s_bps - (momentum_move_bps / Decimal::from(3_u32))).round_dp(4);
...
fn reference_quote_for_window_from_source(
    quotes: &VecDeque<LiveSpotQuote>,
    latest_event_time_ms: i64,
    window_ms: i64,
    source: SpotQuoteSource,
    ...
    quotes
        .iter()
        .copied()
        .filter(|quote| quote.source == source && quote.event_time_ms <= target_ms)
        .max_by_key(live_quote_freshness_key)
}
...
fn parse_one_minute_kline(raw_kline: &[serde_json::Value]) -> Result<OneMinuteKline> {
    ...
    let Some(open_value) = raw_kline.get(1).and_then(serde_json::Value::as_str) else {
        return Err(AppError::InvalidMarket(
            "Binance kline response is missing the open price".to_owned(),
        ));
    };
    let Some(close_value) = raw_kline.get(4).and_then(serde_json::Value::as_str) else {
        return Err(AppError::InvalidMarket(
            "Binance kline response is missing the close price".to_owned(),
        ));
    };
    let Some(close_time_ms) = raw_kline.get(6).and_then(serde_json::Value::as_i64) else {
        return Err(AppError::InvalidMarket(
            "Binance kline response is missing the close time".to_owned(),
        ));
    };
...
mod tests {
    ...
    #[tokio::test]
    async fn coinbase_ticker_does_not_inject_cross_exchange_momentum() {
        let client = BinanceClient::new(
            "https://example.invalid".to_owned(),
            "wss://example.invalid/ws".to_owned(),
            1,
        ...
        client
            .update_trade_quote(
                "btcusdt",
                &format!(r#"{{"E":{},"p":"100.00"}}"#, now_ms - 6_000),
            )
            .await
            .expect("seed older primary quote");
        client
            .update_trade_quote(
                "btcusdt",
                &format!(r#"{{"E":{},"p":"100.00"}}"#, now_ms - 100),
            )
            .await
            .expect("seed latest primary quote");
        ...
        assert!(
            client
                ...
                .await
                .expect("ingest faster Coinbase quote")
        );
        ...
        assert_eq!(
            ...
            Some(SpotQuoteSource::CoinbaseTicker)
        );
        assert_eq!(state.snapshot.current_spot_price, Decimal::new(10_050, 2));
        assert_eq!(state.snapshot.spot_move_1s_bps, Decimal::ZERO);
        assert_eq!(state.snapshot.spot_move_5s_bps, Decimal::ZERO);
    }
