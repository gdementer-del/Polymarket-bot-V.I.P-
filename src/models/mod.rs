...
impl FromStr for MarketTarget {
    ...
    fn from_str(value: &str) -> std::result::Result<Self, Self::Err> {
        match value.trim().to_ascii_lowercase().as_str() {
            ...
            _ => Err(format!("unsupported market target `{value}`")),
        }
    }
}
...
impl TargetPriceSource {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::ChainlinkRtdsBasisAdjusted => "chainlink_basis",
    ...
    /// Return `true` when a Chainlink/Polymarket oracle price participates in the anchor.
    #[must_use]
    pub const fn is_oracle_backed(self) -> bool {
        matches!(
            self,
            Self::PolymarketEventMetadata
                | Self::ChainlinkRtdsWindowOpen
                | Self::ChainlinkRtdsBasisAdjusted
        )
    }
}
...
impl GammaMarket {
    ...
    fn into_binary_market_with_options(self, require_active: bool) -> Result<Option<BinaryMarket>> {
        ...
        let end_date = self
            ...
            .transpose()
            .map_err(|value| AppError::InvalidMarket(format!("invalid end date `{value}`")))?;
    ...
    fn parse_json_array(field: &'static str, value: Option<String>) -> Result<Vec<String>> {
        value
            ...
            .transpose()?
            .ok_or_else(|| AppError::InvalidMarket(format!("missing field `{field}`")))
    }
}
    ...
    D: Deserializer<'de>,
{
    ...
    decimal_from_value(&value).ok_or_else(|| de::Error::custom("invalid decimal value"))
}
    ...
    D: Deserializer<'de>,
{
    ...
    if value.is_null() {
        ...
        decimal_from_value(&value)
            .map(Some)
            .ok_or_else(|| de::Error::custom("invalid decimal value"))
    }
}
