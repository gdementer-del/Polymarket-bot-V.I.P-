...
pub enum Command {
    ...
    Menu,
    /// Scan active short crypto markets and show the best opportunities.
    Scan {
        /// Number of opportunities to show.
        #[arg(long, default_value_t = 10)]
        top: usize,
    },
    /// Show active short crypto markets and their current quotes.
    Markets {
        ...
        refresh_secs: u64,
        /// Limit refresh cycles. Useful for smoke tests.
        #[arg(long)]
        cycles: Option<usize>,
    },
    /// Open a terminal dashboard with market and signal summaries.
    Dashboard {
        /// Number of market rows to show.
    ...
    /// Run the strategy against the available local historical sample.
    Backtest {
    ...
    /// Run the strategy against historical `PolyBackTest` snapshots.
    ...
    /// Sweep PolyBacktest parameters and rank the variants.
    ...
    /// Show analytics for the local execution journal.
    Analytics {
        /// Maximum number of recent journal records to analyze.
        #[arg(long)]
        limit: Option<usize>,
    },
    /// Show a summary of the local paper journal.
    #[command(name = "paper-report", alias = "paper_report")]
    PaperReport {
        #[arg(long)]
        limit: Option<usize>,
    },
    /// Show paper-trade open and close events.
    ...
    /// Show current open paper positions.
    #[command(name = "paper-positions", alias = "paper_positions")]
    PaperPositions,
    /// Validate live Polymarket credentials without sending orders.
    AuthCheck,
    ...
    Run {
        /// Override the execution mode from the configuration file.
        #[arg(long)]
        mode: Option<BotMode>,
        ...
        max_drain_secs: u64,
        /// Redraw a compact paper-session status dashboard in place.
        #[arg(long)]
...
pub enum BotMode {
    ...
    Paper,
    /// Send real orders through the official Polymarket Rust SDK.
    Live,
}
...
impl AppConfig {
    ...
    pub(crate) fn validate(&self) -> Result<()> {
        if self.strategy.max_markets == 0 {
            return Err(AppError::InvalidConfig(
                "strategy.max_markets must be greater than zero",
            ));
        }

        if self.strategy.market_targets.is_empty() {
            return Err(AppError::InvalidConfig(
                "strategy.market_targets must not be empty",
            ));
        }

        if self.run.execute_top_n == 0 {
            return Err(AppError::InvalidConfig(
                "run.execute_top_n must be greater than zero",
            ));
        }
        ...
        if self.strategy.codex_scalp_probe_v1_raw_light_enabled
            && self
                ...
                .market_targets
                .iter()
                .any(|target| target.polymarket_chainlink_symbol().is_none())
        {
            return Err(AppError::InvalidConfig(
                "strategy.codex_scalp_probe_v1_raw_light_enabled requires public Chainlink RTDS support for every market target",
            ));
        }
        ...
        if self.run.chainlink_oracle.enabled
            && (self.run.chainlink_oracle.max_quote_age_ms <= 0
                ...
                || self.run.chainlink_oracle.max_live_context_open_lag_ms < 0
        ...
        if self.run.adaptive_regime.safe_max_entries_per_cycle == 0 {
            return Err(AppError::InvalidConfig(
                "run.adaptive_regime.safe_max_entries_per_cycle must be greater than zero",
            ));
        }
        ...
        {
            return Err(AppError::InvalidConfig(
                "run.adaptive_regime.safe_max_bundle_cost must be in range (0, 2)",
            ));
        }
        ...
        {
            return Err(AppError::InvalidConfig(
                "run.adaptive_regime.aggressive_max_bundle_cost must be in range (0, 2)",
            ));
        }

        if self.run.risk.max_daily_loss_usdc < Decimal::ZERO {
            return Err(AppError::InvalidConfig(
                "run.risk.max_daily_loss_usdc must not be negative",
            ));
        }

        if self.run.risk.max_session_loss_usdc < Decimal::ZERO {
            return Err(AppError::InvalidConfig(
                "run.risk.max_session_loss_usdc must not be negative",
            ));
        }
        ...
        {
            return Err(AppError::InvalidConfig(
                "run.risk.cooldown_cycles must be greater than zero when risk limits are enabled",
            ));
        }
        ...
        if self.strategy.max_bundle_notional_usdc <= Decimal::ZERO {
            return Err(AppError::InvalidConfig(
                "strategy.max_bundle_notional_usdc must be positive",
            ));
        }

        if self.strategy.max_directional_notional_usdc <= Decimal::ZERO {
            return Err(AppError::InvalidConfig(
                "strategy.max_directional_notional_usdc must be positive",
            ));
        }

        if self.strategy.directional_soft_entry_min_notional_usdc <= Decimal::ZERO {
            return Err(AppError::InvalidConfig(
                "strategy.directional_soft_entry_min_notional_usdc must be positive",
            ));
        }

        if self.strategy.directional_soft_entry_max_notional_usdc <= Decimal::ZERO {
            return Err(AppError::InvalidConfig(
                "strategy.directional_soft_entry_max_notional_usdc must be positive",
            ));
        }
        ...
        {
            return Err(AppError::InvalidConfig(
                "strategy.directional_soft_entry_min_notional_usdc must not exceed strategy.directional_soft_entry_max_notional_usdc",
            ));
        }
        ...
        {
            return Err(AppError::InvalidConfig(
                "strategy.directional_soft_entry_max_notional_usdc must not exceed strategy.max_directional_notional_usdc",
            ));
        }

        if self.strategy.micro_breakout_min_spot_move_5s_bps < Decimal::ZERO {
            return Err(AppError::InvalidConfig(
                "strategy.micro_breakout_min_spot_move_5s_bps must not be negative",
            ));
        }
        ...
        if self.strategy.micro_breakout_weak_notional_usdc < Decimal::ZERO {
            return Err(AppError::InvalidConfig(
                "strategy.micro_breakout_weak_notional_usdc must not be negative",
            ));
        }
        ...
        {
            return Err(AppError::InvalidConfig(
                "strategy.micro_breakout_normal_notional_usdc must not be less than strategy.micro_breakout_weak_notional_usdc",
            ));
        }
        ...
        {
            return Err(AppError::InvalidConfig(
                "strategy.micro_breakout_normal_notional_usdc must not exceed strategy.max_directional_notional_usdc",
            ));
        }
        ...
        {
            return Err(AppError::InvalidConfig(
                "strategy.micro_breakout_strong_notional_usdc must not be less than strategy.micro_breakout_normal_notional_usdc",
            ));
        }
        ...
        {
            return Err(AppError::InvalidConfig(
                "strategy.micro_breakout_strong_notional_usdc must not exceed strategy.max_directional_notional_usdc",
            ));
        }
        ...
        {
            return Err(AppError::InvalidConfig(
                "strategy.micro_breakout_full_size_max_entry_price must be in range (0, 1)",
            ));
        }
        ...
        {
            return Err(AppError::InvalidConfig(
                "strategy.micro_breakout_full_size_max_entry_price must not exceed strategy.micro_breakout_expensive_entry_price",
            ));
        }
        ...
        {
            return Err(AppError::InvalidConfig(
                "strategy.micro_breakout_strong_signal_min_spot_move_5s_bps must not be negative",
            ));
        }
        ...
        {
            return Err(AppError::InvalidConfig(
                "strategy.micro_breakout_strong_signal_min_spot_move_15s_bps must not be negative",
            ));
        }
        ...
        {
            return Err(AppError::InvalidConfig(
                "strategy.micro_breakout_min_elapsed_window_secs must be in range [0, 300]",
            ));
        }
        ...
        if self.strategy.directional_strong_signal_min_spot_move_5s_bps < Decimal::ZERO {
            return Err(AppError::InvalidConfig(
                "strategy.directional_strong_signal_min_spot_move_5s_bps must not be negative",
            ));
        }

        if self.strategy.directional_strong_signal_min_trade_flow_bps < Decimal::ZERO {
            return Err(AppError::InvalidConfig(
                "strategy.directional_strong_signal_min_trade_flow_bps must not be negative",
            ));
        }

        if self.strategy.directional_soft_entry_signal_window_bps < Decimal::ZERO {
            return Err(AppError::InvalidConfig(
                "strategy.directional_soft_entry_signal_window_bps must not be negative",
            ));
        }
        ...
        {
            return Err(AppError::InvalidConfig(
                "strategy.max_seconds_left must be greater than strategy.min_seconds_left",
            ));
        }
        ...
        if self.strategy.max_market_notional_usdc < largest_trade_limit {
            return Err(AppError::InvalidConfig(
                "strategy.max_market_notional_usdc must not be less than the largest single-trade size",
            ));
        }
        ...
        {
            return Err(AppError::InvalidConfig(
                "strategy.directional_max_entry_price must be in range (0, 1)",
            ));
        }
        ...
        {
            return Err(AppError::InvalidConfig(
                "strategy.directional_max_fair_price must be in range (0.50, 1.00]",
            ));
        }

        if self.strategy.directional_projection_cap_multiplier < Decimal::ONE {
            return Err(AppError::InvalidConfig(
                "strategy.directional_projection_cap_multiplier must be at least 1",
            ));
        }

        if self.strategy.directional_trade_flow_weight < Decimal::ZERO {
            return Err(AppError::InvalidConfig(
                "strategy.directional_trade_flow_weight must not be negative",
            ));
        }

        if self.strategy.directional_execution_slippage_bps > 2_500 {
            return Err(AppError::InvalidConfig(
                "strategy.directional_execution_slippage_bps must not exceed 2500 bps",
            ));
        }
        ...
        {
            return Err(AppError::InvalidConfig(
                "strategy.tail_hedge_ratio must be in range [0, 1]",
            ));
        }
        ...
        {
            return Err(AppError::InvalidConfig(
                "strategy.tail_hedge_max_opposite_price must be in range (0, 1)",
            ));
        }
        ...
        {
            return Err(AppError::InvalidConfig(
                "strategy.tail_hedge_max_bundle_cost must be in range (0, 2)",
            ));
        }
        ...
        {
            return Err(AppError::InvalidConfig(
                "strategy.tail_hedge_open_window_secs must be in range [0, 300]",
            ));
        }
    ...
    fn validate_codex_scalp_probe_v1(&self) -> Result<()> {
        if self.strategy.codex_scalp_probe_v1_raw_light_threshold_scale <= Decimal::ZERO {
            return Err(AppError::InvalidConfig(
                "strategy.codex_scalp_probe_v1_raw_light_threshold_scale must be positive",
            ));
        }
        ...
        if self.strategy.codex_scalp_probe_v1_min_fill_ratio < Decimal::ZERO
            || self.strategy.codex_scalp_probe_v1_min_fill_ratio > Decimal::ONE
        {
            return Err(AppError::InvalidConfig(
                "strategy.codex_scalp_probe_v1_min_fill_ratio must be in range [0, 1]",
            ));
        }
...
pub struct StrategyConfig {
    ...
    #[serde(default = "default_codex_scalp_probe_v1_raw_light_threshold_scale")]
    pub codex_scalp_probe_v1_raw_light_threshold_scale: Decimal,
    ...
    #[serde(default = "default_codex_scalp_probe_v1_min_fill_ratio")]
    pub codex_scalp_probe_v1_min_fill_ratio: Decimal,
...
pub struct ChainlinkOracleConfig {
    ...
    #[serde(default = "default_chainlink_oracle_max_live_context_open_lag_ms")]
    pub max_live_context_open_lag_ms: i64,
...
impl Default for ChainlinkOracleConfig {
    fn default() -> Self {
        Self {
            ...
            max_live_context_open_lag_ms: default_chainlink_oracle_max_live_context_open_lag_ms(),
...
fn default_codex_scalp_probe_v1_raw_light_threshold_scale() -> Decimal {
    Decimal::ONE
}
...
fn default_codex_scalp_probe_v1_min_fill_ratio() -> Decimal {
    Decimal::ZERO
}
...
const fn default_chainlink_oracle_max_live_context_open_lag_ms() -> i64 {
    30_000
}
...
mod tests {
    ...
    fn codex_sentinel_config_validates_without_legacy_v2() {
        let raw = include_str!("../config.sentinel.toml");
    ...
    fn codex_scalp_config_validates_runtime_scalp_exit() {
        let raw = include_str!("../config.scalp-v1.toml");
        let config: AppConfig = toml::from_str(raw).expect("fixture config should parse");
    ...
    fn codex_scalp_raw_config_is_paper_only_ablation() {
        let raw = include_str!("../config.scalp-v1-raw.toml");
        let config: AppConfig = toml::from_str(raw).expect("fixture config should parse");
    ...
    fn codex_scalp_raw_light_config_validates_market_specific_probe() {
        let raw = include_str!("../config.scalp-v1-raw-light-v2.toml");
        let config: AppConfig = toml::from_str(raw).expect("fixture config should parse");
    ...
    fn codex_scalp_raw_light_v3_config_targets_multi_asset_scalp_bucket() {
        let raw = include_str!("../config.scalp-v1-raw-light-v3.toml");
        let config: AppConfig = toml::from_str(raw).expect("fixture config should parse");
    ...
    fn codex_scalp_raw_light_requires_raw_ablation() {
        let raw = include_str!("../config.scalp-v1-raw-light-v2.toml").replace(
    ...
    #[test]
    fn codex_scalp_raw_light_rejects_target_without_public_chainlink_anchor() {
        let raw = include_str!("../config.scalp-v1-raw-light-v3.toml").replace(
            r#"market_targets = ["btc-5m", "eth-5m", "sol-5m", "xrp-5m"]"#,
            r#"market_targets = ["btc-5m", "eth-5m", "sol-5m", "xrp-5m", "bnb-5m"]"#,
        );
        let config: AppConfig = toml::from_str(&raw).expect("fixture config should parse");
        let err = config
            .validate()
            .expect_err("raw-light mode must reject targets without a public oracle anchor");

        assert!(
            err.to_string()
                .contains("requires public Chainlink RTDS support for every market target")
        );
    }
    ...
    fn codex_scalp_boost_config_validates_pnl_sizing_controls() {
        let raw = include_str!("../config.scalp-v1-boost.toml");
        let config: AppConfig = toml::from_str(raw).expect("fixture config should parse");
    ...
    fn codex_v4_champion_config_limits_repeat_window_risk() {
        let raw = include_str!("../config.v4-champion.toml");
    ...
    fn config_rejects_codex_sentinel_with_legacy_v2_enabled() {
        let raw = include_str!("../config.sentinel.toml").replace(
