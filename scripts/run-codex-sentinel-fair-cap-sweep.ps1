param(
    [string[]]$Caps = @("0.80", "0.82", "0.84", "0.86", "0.88"),
    [int]$WindowsPerTarget = 5,
    [int[]]$EntryMinutes = @(1),
    [int]$Top = 10,
    [string]$Target = "btc-5m",
    [string]$BaseConfig = "config.sentinel.toml",
...
function New-SweepConfig {
    ...
    $fairCapPattern = '(?m)^\s*bonereaper_state_v2_max_fair_price\s*=\s*".*"\s*$'
    if ($text -notmatch $fairCapPattern) {
        throw "Base config must contain bonereaper_state_v2_max_fair_price."
    }
    $text = [regex]::Replace(
        $text,
        $fairCapPattern,
        "bonereaper_state_v2_max_fair_price = `"$Cap`""
    )
...
$runRoot = Join-Path $repoRoot "state\polybacktest-runs\$stamp-fair-cap-sweep"
New-Item -ItemType Directory -Force -Path $runRoot | Out-Null

$summaryPath = Join-Path $runRoot "summary.csv"
"cap,config,entry_minutes,target,windows,signals,near_misses,expected,realized,hit_rate_pct,status,log" |
...
try {
    foreach ($cap in $Caps) {
        $safeCap = $cap.Replace(".", "p")
        $configPath = Join-Path $runRoot "sentinel-fair-cap-$safeCap.toml"
        $stateDir = "state/polybacktest-runs/$stamp-fair-cap-sweep/state-cap-$safeCap"
        New-SweepConfig `
        ...
        foreach ($entryMinute in $EntryMinutes) {
            $logPath = Join-Path $runRoot "cap-$safeCap-entry$entryMinute.log"
            $cargoArgs = @("run")
            if ($Release) {
                $cargoArgs += "--release"
            }
            ...
            Write-Host "Running fair cap=$cap entry=$entryMinute target=$Target windows=$WindowsPerTarget"
            ...
            "$(ConvertTo-CsvCell $cap),$(ConvertTo-CsvCell $configPath),$entryMinute,$Target,$WindowsPerTarget,$signals,$nearMisses,$expected,$realized,$hitRate,$status,$(ConvertTo-CsvCell $logPath)" |
...
Write-Host "Fair-cap sweep complete: $summaryPath"
