...
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\compare-strategy-configs.ps1 `
  -BaselineConfig .\config.codex-scalp-v1.toml `
  -CandidateConfig .\config.codex-scalp-v1.toml `
...
```powershell
.\target\debug\polymarket_mvp.exe --config .\config.codex-scalp-v1.toml run --mode paper --max-runtime-secs 1800 --drain-open-positions --max-drain-secs 300
```
