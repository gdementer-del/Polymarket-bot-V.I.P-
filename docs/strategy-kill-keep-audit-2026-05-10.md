...
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| `config.codex-bonereaper-hot.toml` | 100 | 0 | 0/0 | 0.0000 | 0.00% | Too restrictive / no validation sample |
| `config.codex-sentinel.toml` | 100 | 3 | 2/1 | +3.8805 | 66.67% | Only live branch, but massively under-sampled |
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| `config.codex-sentinel.toml` | 500 | 99 | 4 | 3/1 | +6.8640 | +4.4872 | 75.00% | Positive but still far too under-sampled |
...
| `config.codex-sentinel.toml` | 20 | 7 | -1.5236 | 57.14% | Too small |
| `config.codex-bonereaper-hot.toml` | 20 | 4 | -11.3793 | 25.00% | Too small, negative |
...
target\debug\polymarket_mvp.exe --config config.codex-sentinel.toml polybacktest --windows-per-target 500 --entry-minutes 1 --target btc-5m
```
